...
type ListingWithBio = {
  id: string;
  hostBio: string;
  hostBioI18n?: { ko?: string; ja?: string; zh?: string };
};

/**
 * ja/zh일 때 hostBio 번역이 없으면 번역 후 DB에 캐시하고 listing 객체에 반영
...
export async function ensureHostBioTranslated<T extends ListingWithBio>(
  ...
  if (lang !== "ja" && lang !== "zh") return listing;

  const source = listing.hostBio?.trim();
  if (!source || source.length < 10) return listing;

  const cached =
    lang === "ja" ? listing.hostBioI18n?.ja?.trim() : listing.hostBioI18n?.zh?.trim();
  ...
  try {
    await prisma.listing.update({
      where: { id: listing.id },
      data: lang === "ja" ? { hostBioJa: translated } : { hostBioZh: translated },
    });
  } catch (e) {
