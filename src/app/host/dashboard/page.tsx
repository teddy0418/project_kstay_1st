...
import { getHostListingsForCalendar, getHostDashboardStats } from "@/lib/repositories/host-calendar";
import { getHostAnnouncements } from "@/lib/repositories/host-announcements";
import DashboardListingSelector from "./DashboardListingSelector";
...
export default async function HostDashboardPage(props: PageProps) {
  ...
  /** null = 전체(기본값), 있으면 해당 숙소만 */
  const listingId =
    listingIdParam && listings.some((l) => l.id === listingIdParam)
      ? listingIdParam
      : null;

  const [stats, centerFeed] = await Promise.all([
  ...
  const summary: { label: string; value: string | number; danger?: boolean }[] = [
    ...
    { label: "읽지 않은 메시지", value: stats.newMessages },
  ...
  const currentListing = listingId ? listings.find((l) => l.id === listingId) : null;
  ...
  return (
    <div className="grid gap-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        {listings.length >= 1 && (
          <DashboardListingSelector listings={listings} currentListingId={listingId} />
        )}
      </div>
      {currentListing && (
        <p className="text-xs text-neutral-500">현재 기준: {currentListing.title}</p>
      )}
