...
/** POST { date: "YYYY-MM-DD" } — 해당 날짜를 호스트의 모든 승인 숙소에 대해 판매 중지(차단)합니다. */
export async function POST(req: Request) {
  ...
  for (const listing of listings) {
    try {
      await blockDate(listing.id, user.id, date);
    } catch {
      // skip if single listing fails (e.g. already blocked)
    }
  }

  return apiOk({ ok: true, blockedCount: listings.length });
}
