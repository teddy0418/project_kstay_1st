...
export default function Header() {
  ...
  console.log("[KSTAY] 5. Header render (client)", { pathname });
  ...
  const isLoggedIn = false; // MVP placeholder
          ...
          <div className="relative" ref={menuRef}>
            ...
            {openMenu && (
              <div className="absolute right-0 mt-2 w-64 rounded-2xl border border-neutral-200 bg-white shadow-elevated overflow-hidden">
                ...
                <Link href="/wishlist" className="block px-4 py-3 text-sm hover:bg-neutral-50" onClick={() => setOpenMenu(false)}>
                  Wishlist
                </Link>
                <Link href="/messages" className="block px-4 py-3 text-sm hover:bg-neutral-50" onClick={() => setOpenMenu(false)}>
                  Messages
                </Link>
                <Link href="/profile" className="block px-4 py-3 text-sm hover:bg-neutral-50" onClick={() => setOpenMenu(false)}>
                  Profile
                </Link>

                <div className="h-px bg-neutral-200" />

                <Link href="/host" className="block px-4 py-3 text-sm hover:bg-neutral-50" onClick={() => setOpenMenu(false)}>
                  Host Partners
                </Link>
                <Link href="/admin" className="block px-4 py-3 text-sm hover:bg-neutral-50" onClick={() => setOpenMenu(false)}>
                  Admin
                </Link>
                ...
                <button
                  ...
                  onClick={() => {
                    setOpenMenu(false);
                    if (!isLoggedIn) toast("You are not logged in yet. (MVP)");
                    else toast("Logged out. (MVP)");
                  }}
