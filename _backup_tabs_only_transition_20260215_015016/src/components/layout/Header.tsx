...
import { Globe, Menu, Home, MapPin, CheckCircle2, LogOut, Heart, MessageCircle, User, Building2, Shield } from "lucide-react";
...
export default function Header() {
  ...
  const guard = (nextPath: string, desiredRole: "guest" | "host" | "admin" = "guest") => {
    if (!loggedIn) {
      openAuth({ next: nextPath, role: desiredRole });
      return;
    }
              ...
              <div className="absolute right-0 mt-2 w-64 rounded-2xl border border-neutral-200 bg-white shadow-elevated overflow-hidden">
                {!loggedIn ? (
                  <button
                    type="button"
                    onClick={() => {
                      openAuth({ next: pathname || "/", role: "guest" });
                      setOpenMenu(false);
                    }}
