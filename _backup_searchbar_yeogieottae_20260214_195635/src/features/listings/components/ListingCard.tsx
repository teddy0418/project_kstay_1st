...
export default function ListingCard({ listing, active, onHoverChange }: ListingCardProps) {
  const { currency } = useCurrency();
  const { toast } = useToast();
  console.log("[KSTAY] 7. ListingCard render (client)", { id: listing.id, title: listing.title });
  ...
  return (
    <div
      className={cn("group relative", active && "ring-2 ring-brand rounded-2xl")}
    ...
    >
      ...
      <button
        ...
        className="group/heart absolute right-3 top-3 inline-flex h-10 w-10 items-center justify-center rounded-full bg-white/90 shadow-soft transition active:scale-95 relative"
